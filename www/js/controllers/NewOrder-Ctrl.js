    ...
    function NewOrderCtrl($scope, $state, $stateParams, $ionicLoading, Orders, Customers, Products) {
        if (typeof $stateParams.orderId != 'undefined') {
            ...
            Orders.getOrderbyId().then(function (data) {
                $scope.neworder = data;
                Customers.getCustomerbyId($scope.neworder.custid).then(function (data) {
                    $scope.neworder.custname = data.name;
                    $scope.neworder.custphone = data.phone;
                    $scope.neworder.custemail = data.email;
                    $scope.neworder.custaddress = data.address1 + ", " + data.address2 + ", " + data.city;
                    $ionicLoading.hide();
                });
        ...
        if ($scope.neworder != null || $scope.neworder.product != null || $scope.neworder.product != undefined) {
